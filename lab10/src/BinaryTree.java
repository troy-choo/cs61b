...
public class BinaryTree<T> {
    ...
    private boolean isCompletelyBalanced(TreeNode<T> node) {
        if (node == null) {
            ...
            int leftHeight = height(node.getLeft());
            int rightHeight = height(node.getRight());
            boolean isLeftBalanced = isCompletelyBalanced(node.getLeft());
            boolean isRightBalanced = isCompletelyBalanced(node.getRight());

            return (leftHeight == rightHeight) && isLeftBalanced && isRightBalanced;
        }
    }
    ...
    public static BinaryTree<Integer> fibTree(int N) {
        if (N == 0) {
            BinaryTree<Integer> baseCase = new BinaryTree<Integer>(new TreeNode<>(0));
            return baseCase;
        }
        else if (N == 1) {
            BinaryTree<Integer> baseCase = new BinaryTree<Integer>(new TreeNode<>(1));
            return baseCase;
        } else {
            ...
            BinaryTree<Integer> result = new BinaryTree<Integer>(new TreeNode(rootValue, leftSubTree.getRoot(), rightSubTree.getRoot()));
            return result;
        }
    }
