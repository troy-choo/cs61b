...
public class Graph {
    ...
    public Graph prims(int start) {
        Graph mst = new Graph();
        Set<Integer> visited = new HashSet<>();
        Map<Integer, Edge> edgeToVertex = new HashMap<>();
        PriorityQueue<Edge> fringe = new PriorityQueue<>(Comparator.comparingInt(Edge :: getWeight));
        ...
        for (Edge e : getEdges(start)) {
            fringe.offer(e);
            edgeToVertex.put(e.getDest(), e);
        }

        while (!fringe.isEmpty()) {
            Edge smallestEdge = fringe.poll();
            int v = smallestEdge.getDest();

            if (!visited.contains(v)) {
                visited.add(v);
                mst.addVertex(v);
                mst.addEdge(edgeToVertex.get(v));

                for (Edge e : getEdges(v)) {
                    if (!visited.contains(e.getDest())) {
                        if (!edgeToVertex.containsKey(e.getDest()) || edgeToVertex.get(e.getDest()).getWeight() > e.getWeight()) {
                            edgeToVertex.put(e.getDest(), e);
                        }
                    }
                    fringe.offer(e);
    ...
    public Graph kruskals() {
        Graph mst = new Graph();
        WeightedQuickUnionUF unionFind = new WeightedQuickUnionUF(getAllVertices().size());
